...
def update_tables():
    ...
    if data:
        ...
        if product_name == "Water":
            ...
            if availability == 0 and ("Water" not in product1_df["Product Name"].values or product1_df.loc[product1_df["Product Name"] == "Water", "Stock"].iloc[-1] != 0):
                product1_df = pd.concat([pd.DataFrame([data], columns=["Timestamp", "Product Name", "Price", "Stock", "Action"]), product1_df], ignore_index=True)
                product1_table.table(product1_df)
        elif product_name == "Snack":
            # Check if this is the first time availability reaches 0
            if availability == 0 and ("Snack" not in product2_df["Product Name"].values or product2_df.loc[product2_df["Product Name"] == "Snack", "Stock"].iloc[-1] != 0):
                product2_df = pd.concat([pd.DataFrame([data], columns=["Timestamp", "Product Name", "Price", "Stock", "Action"]), product2_df], ignore_index=True)
                product2_table.table(product2_df)
        elif product_name == "Instant Noodle":
            # Check if this is the first time availability reaches 0
            if availability == 0 and ("Instant Noodle" not in product3_df["Product Name"].values or product3_df.loc[product3_df["Product Name"] == "Instant Noodle", "Stock"].iloc[-1] != 0):
                product3_df = pd.concat([pd.DataFrame([data], columns=["Timestamp", "Product Name", "Price", "Stock", "Action"]), product3_df], ignore_index=True)
